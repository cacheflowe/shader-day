...
var AudioHandler = function() {
	...
	//load sample MP3
	function loadSampleAudio(audioFile) {
	...
	function stopSound(){
		...
		if(debugCtx) debugCtx.clearRect(0, 0, debugW, debugH);
	}
	...
	function onUseSample(audioFile){
		if (usingAudioFile == false){
			usingAudioFile = true;
			usingMic = false;
			loadSampleAudio(audioFile);
		}else{
			stopSound();
		}
	}
	...
	function onBeat(){
		...
		if (fakeBpmMode == true) return;

		events.emit("onBeat");
		console.log('onBeat emitted');
	}
	...
	function update(){
		...
		if (showingDebug && debugCtx) debugDraw();
	}
